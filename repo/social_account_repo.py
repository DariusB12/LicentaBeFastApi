...
from model.entities import SocialMediaAccount, User
...
def mark_social_account_as_modified(social_account_id: int, db: Session):
    """
    Mark the given social account as modified by the user
    :param social_account_id: the id of the social account
    :param db: the db connection
    :return: none
    """
    db.query(SocialMediaAccount).filter(SocialMediaAccount.id == social_account_id).update(
        {'modified': True})
    db.commit()
